...
var ChatCohere = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	...
	constructor(modelOrFields, fields) {
		...
		super(params);
		this._addVersion("@langchain/cohere", "1.1.0");
	...
	async *_streamChatModelEvents(messages, options, _runManager) {
		const request = this._getChatRequest(messages, options);
		const stream = await this.caller.callWithOptions({ signal: options.signal }, async () => this.client.chatStream(request));
		...
		yield* require_stream_events.convertCohereStream(abortableStream(stream, options.signal), { streamUsage: shouldStreamUsage });
	}
